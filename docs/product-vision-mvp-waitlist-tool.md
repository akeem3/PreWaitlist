...
| Milestone rewards configuration                                                                                                                     | [Founder] sets · [Subscriber] earns       | 🟢 Should | Founder configures visible rewards at specific referral thresholds during page setup (onboarding Step 3). Example: "Refer 3 → early access / Refer 10 → founding member badge / Refer 25 → free Pro for 3 months." Displayed on thank-you page above share buttons before the share ask — Hopkins principle: give before asking. Elevated from ⚪ v1.1 based on field research Finding 13 (thank-you page is the primary referral loop activation surface) and Finding 9 (documented conversion lift from visible reward before ask) |
...
| Feature                                                    | User                                                    | Grade     | Why                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------- | ------------------------------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Warmth score per subscriber (Hot / Warm / Cold)            | [Founder] sees · calculated from [Subscriber] behaviour | 🔵 Core   | Calculated from: email opens (Resend webhooks), page return visits (Supabase), qualification answers completed, founder update views. Three states. Colour-coded in the subscriber list. Technically simple to build                                 |
...
| Automated warmth alerts ("40% of your list is going cold") | [Founder]                                               | 🟢 Should | Sends an email to the founder when cold % crosses a threshold. Growth tier fence feature. Solo founders check manually; small teams need the tool to monitor for them. Technically simple — a cron job checking cold % and triggering a Resend email |
...
| Email open rate and click tracking                 | [Founder]                                        | 🟢 Should | Resend webhooks feed email open signals into warmth scoring. Elevated from ⚪ v1.1 — warmth tracking is materially incomplete without this signal. Without open tracking, a subscriber who opened every email still shows as Warm, not Hot. Technical implementation is simple: Resend provides open/click webhooks natively. Required for warmth scoring accuracy                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Paddle subscription integration (Pro $15/mo, Growth $29/mo) | [Founder]         | 🔵 Core   | Revenue. Handles all tax/VAT globally. No US entity needed                         |
...
| Referral system        | Unique referral link per subscriber, position tracking, automatic recalculation, leaderboard (count + quality score), thank-you page with pre-filled share buttons and milestone reward ladder                                                                                  |
| Warmth tracking        | Hot/Warm/Cold per subscriber, email open tracking via Resend webhooks (elevated from v1.1 — required for warmth accuracy), warmth filter in subscriber list, warmth-segmented broadcast, automated warmth alerts (Growth tier — email to founder when cold % crosses threshold) |
| Referral quality score | Engagement-weighted score per referrer alongside raw count in leaderboard                                                                                                                                                                                                       |
| Email                  | Confirmation email, "you moved up" trigger, sender name customisation (Pro/Growth), sender domain authentication SPF/DKIM walkthrough (Pro/Growth), broadcast to full list, warmth-segmented broadcast                                                                          |
...
| Account                | Email auth + Google OAuth, free tier, Pro $15/mo + Growth $29/mo via Paddle                                                                                                                                                                                                     |
...
**Growth ($29/mo)** — The early-stage startup team (Profile 3). Multiple people managing the launch, needs the tool to monitor and alert rather than requiring manual checking, and needs unlimited qualification depth for active customer segmentation.
...
**Why priority support is in Growth:** At $29/month, a startup team running a real launch has real stakes. A broken email trigger or a dashboard bug costs them more than $29.
...
**Duration:** 11 days
**Goal:** Warmth tracking is live and visible. Founders can send warmth-segmented broadcasts. Paddle billing gates the Pro and Growth features. Domain authentication is walkable.
...
| Growth tier automated alert email (founder inbox)           | I-6-EMAIL | 🟢 Should |

**What gets built:**

- Warmth score calculation engine (email open via Resend webhooks + page return visits + qual answers + update views)
...
- Warmth-segmented broadcast compose + send (Pro/Growth)
- Broadcast email to full list (Pro/Growth)
- Paddle integration — Pro $15/mo, Growth $29/mo
- Upgrade modal with 7 context-sensitive trigger variants
- Feature gating across all tiers
- Sender domain authentication walkthrough (SPF/DKIM via Resend — Pro/Growth)
- Automated warmth alert email to founder (Growth tier — cron job)
- Email open tracking via Resend webhooks feeding warmth score

**Exit condition:** A founder on the free tier who hits the 500-signup cap sees the upgrade modal. A Pro founder can send a warmth-segmented broadcast. A Growth founder receives an automated alert when their Cold % crosses the threshold. Domain authentication setup is walkable in the UI.
...
- Second waitlist creation flow (F-D1, F-D2) — lighter path for returning Pro/Growth founders
...
| Email open/click tracking                | Problem Brief ("Email open/click tracking")      | 🟢 Should — MVP, Sprint 3 | Resend webhooks feeding warmth score. Elevated per Product Vision MVP — warmth scoring is materially incomplete without it |
