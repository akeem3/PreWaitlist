...
This story is algorithmic — the decay logic lives inside `src/lib/warmth.ts` created in Story 11.1. Story 11.1's T1 already includes the decay calculation function. This story provides the detailed specification for that function.
...
**Important:** `page_views` table is NOT populated. No code inserts into it. Decay relies solely on `email_events.created_at` for last engagement signal. Page-visit-based warmth scoring is deferred to v1.1 when the tracking middleware is built.
...
2. Test: subscriber with last event 30 days ago → decay = 0
3. Test: subscriber with last event 70 days ago → decay = 25
4. Test: subscriber with last event 100 days ago → decay = 999 (reset to 0)
