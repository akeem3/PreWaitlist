...
> **Dev Notes (2026-09-25 — warmth restructure):** AC1, AC3, and AC4 are superseded — the panel renders **three bars (Hot/Warm/Cold)**, the `Unscored = bg-muted` color row is gone, and `GET /api/dashboard/warmth` returns `{ hot, warm, cold, total }` (no `unscored`). The `/api/warmth/[subdomain]` route referenced below was deleted (orphan). Original AC text retained above for history. **[AMENDED 2026-09-25]**
...
Shipped in `components/dashboard/warmth-panel.tsx`: Hot = `bg-status-hot` (founder override 2026-09-25 reinstated the `bg-status-hot` token over the Story 15.4 `bg-accent` alignment), Warm = `bg-status-warm`, Cold = `bg-status-cold`. Unscored bar removed 2026-09-25 (warmth restructure — three bars total).

### T2: Data flow (AC4)

The panel no longer fetches. Story 12.1.8 moved data fetching to `src/app/dashboard/client.tsx` — one authenticated `GET /api/dashboard/warmth?waitlist_id=…` (guarded by `if (waitlist_id)` since the 15.4 audit) → `warmthData` passed as a prop. The endpoint (`src/app/api/dashboard/warmth/route.ts`) returns `{ hot, warm, cold, total }` as head counts (Story 15.4 AC5; `unscored` removed by the 2026-09-25 warmth restructure). The public `/api/warmth/[subdomain]` route referenced in the original plan was orphaned (audit §2.5) and has been deleted.

### T3: Remove blur/lock + empty state (AC5–AC6)

`LockedOverlay` and blur are removed — free tier sees visible counts + an "Upgrade to target segments" nudge (2026-09-22 decision). The `tier` prop remains (default `"free"`) but only selects the presentation variant: free renders the panel as an upgrade-CTA button containing the same three bars, Pro renders the plain panel (`warmth-panel.tsx`, 2026-09-25 redesign: title is a `<Link>` with `after:absolute` overlay, accent `"{n} subscribers"` meta line hidden when total = 0). The `/dashboard/warmth` page stays Pro-gated (Story 12.3.3). Warning threshold lives in `components/dashboard/warning-banner.tsx` (value copied from settings; the free-page 400 fetch was removed by Story 15.4 AC2). Empty state: `{total > 0 ? count : "—"}` unchanged (AC5).
...
1. Bar colors: Hot = coral red (`bg-status-hot`), Warm = amber, Cold = blue — ~~Unscored = grey~~ (no Unscored bar since 2026-09-25, three bars)
