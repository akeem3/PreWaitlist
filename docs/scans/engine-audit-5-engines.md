...
| #   | Sev | Issue                                                                                                                                                                                     | Evidence                                         |
| --- | --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| 1   | 🔴  | **Cron never scheduled** — `batchRecalculateWarmth` never runs in production; `warmth_score` stays at whatever manual/one-off state; segments + warning + panel show stale/null           | `vercel.json` no `crons`; no workflow/pg_cron    |
| 2   | 🔴  | **Batch referral count page-scoped** — `+15` applied only when a subscriber's referrals share their 500-row page; no `.order()` makes split worse                                         | `warmth.ts:101-146,152`                          |
| 3   | 🔴  | **Decay reads ALL email_events** — `sent`/`delivered` reset the 60-day clock; cold detection dead for anyone receiving any email; referrals never reset decay; contradicts Story 11.5 AC1 | `warmth.ts:54-56` vs `story-11.5` AC1            |
| 4   | 🔴  | **Story 11.2 never delivered where written** — no warmth badge column / filter on any subscriber list (main dashboard has no table); only `/dashboard/warmth` has badge+filter            | grep `WarmthBadge` → 1 file                      |
| 5   | 🟠  | Decay off-by-one: −25 at day 59 (AC: 0–59 free); `penalty_max: 89` dead                                                                                                                   | `warmth.ts:64`                                   |
| 6   | 🟠  | Decayed-to-zero → **Unscored not Cold** → warning/segments undercount disengaged                                                                                                          | `assignTier` `:75-76`                            |
| 7   | 🟠  | **2 of 5 declared signals dead**: `email_reply` (Resend has no reply webhook), `leaderboard_visit` (`page_views` never written) — story still lists them as AC2 points                    | `warmth.ts:6,9`; Resend event-types docs         |
| 8   | 🟠  | Webhook multi-waitlist: `.limit(1).single()` attributes events to arbitrary waitlist; idempotency race (no unique on svix_id)                                                             | `webhooks/resend/route.ts:81-98`                 |
| 9   | 🟠  | **Zero webhook / cron / batch / panel tests** — Story 11.6 AC1/AC3/AC5 unmet; invalid-signature 401 promised vs 400 shipped                                                               | glob; story 11.6                                 |
| 10  | 🟠  | Settings helper copy describes score cutoff; actually cold-% warning threshold                                                                                                            | `settings/client.tsx:335`                        |
| 11  | 🟠  | Segments: `.single()` multi-waitlist break, no `?wid=`, no `requirePro`, counts include unsub/bounced                                                                                     | `segments/route.ts`                              |
| 12  | 🟡  | Hot bar `bg-status-hot` (red-orange) ≠ Hot badge green — Story 11.3 AC3                                                                                                                   | `warmth-panel.tsx:113` vs `warmth/client.tsx:49` |
| 13  | 🟡  | Story 11.3 AC6 vs 12.3.3 AC6 + 2026-09-22 decision unreconciled in story text                                                                                                             | story files                                      |
| 14  | 🟡  | WarningBanner no-`waitlist_id` fallback → 400 (latent)                                                                                                                                    | `warning-banner.tsx:41`                          |
| 15  | 🟡  | Public warmth API orphaned                                                                                                                                                                | only `api/warmth.test.ts` imports                |
| 16  | 🟡  | Dashboard warmth API full-table load into JS (12.1.9 pattern)                                                                                                                             | `dashboard/warmth/route.ts:38-47`                |
| 17  | 🟡  | Epic/story status drift: index `ready`, Dev Notes `NOT STARTED`, MEMORY `done`                                                                                                            | epic-11 + story frontmatter                      |
...
| 20  | 🟢  | Story 11.0/11.6 say 401; code 400                                                                                                                                                         | AC drift only                                    |

### 2.5 Tests inventory

| Test file                                        | Covers                                                                                                                                                                                                                  | Gap                                                                                                                                                          |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `lib/warmth.test.ts` (19)                        | clicks, referrals, qual, multi-signal, clamp 0/100, decay 30/70/95d, most-recent-event, non-click ignore, assignTier bounds                                                                                             | **No day-59/60 boundary test** (would catch off-by-one); no batch path; decay tests use only `clicked` events — never assert `sent`/`delivered` are excluded |
...
| **Missing entirely**                             | **webhook route** (11.6 AC1); **cron route**; **`batchRecalculateWarmth`** (referral bug, pagination); **WarmthPanel bar widths/colors** (11.6 AC5); badge/filter as separate 11.6 files; WarningBanner threshold logic | Largest holes: webhook, batch, panel                                                                                                                         |
...
- **What works:** webhook ingestion (with side effects), schema, pure `calculateWarmthScore`/`assignTier` (19 tests), panel/stat-card/warning/settings UI, Pro warmth page, segment-aware broadcast send, free-tier number visibility.
...
- Tests: `warmth.test.ts` (19), `dashboard-warmth-page.test.tsx` (13), `api/warmth.test.ts` (4) — **missing webhook/cron/batch/panel**
