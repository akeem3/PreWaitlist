...
| 3   | ~~**Decayed-to-zero (score 0 with lifetime engagement) → Cold**, not Unscored; Unscored = never engaged / null column~~ **OVERTURNED 2026-09-25 (warmth restructure):** Unscored removed entirely; everyone starts Hot at baseline 70; `warmth_score` never null | Was: fixes warning% and segment undercount of fully lapsed subscribers — restructure makes the question moot (everyone is always classified) |
...
- AC4: ~~`assignTier` (or score+tier helper) shall return `hot` for score ≥ 70, `warm` for ≥ 40, `cold` for score > 0 **or** (score === 0 **and** the subscriber has lifetime engagement: clicks > 0, referrals > 0, or qual answers present), and `null` (Unscored) only when score is 0 **and** there is no lifetime engagement.~~ **[AMENDED 2026-09-25 — warmth restructure:** `assignTier(score)` shall return `hot` for ≥ 70, `warm` for ≥ 40, `cold` otherwise — no `null`, no `hadEngagement`; fresh signup scores 70 (Hot)**]**
...
- AC7: ~~The batch shall continue to write only the tier string (`hot`/`warm`/`cold`/`null`) to `subscribers.warmth_score` and return `{ processed, hot, warm, cold, unscored }`.~~ **[AMENDED 2026-09-25:** the batch writes only the tier string (`hot`/`warm`/`cold` — never `null`) and returns `{ processed, hot, warm, cold }`**]**
...
- **T5:** Update `src/__tests__/lib/warmth.test.ts` in the **same** PR as score changes (do not leave red). Add cases: sent/delivered do **not** reset decay; day 59 no penalty; day 60 −25; ~~decayed engaged zero → cold; never-engaged zero → null~~ **[amended 2026-09-25:** any score < 40 → `cold`, `assignTier` never `null`**]**; zero-click subscriber with old `created_at` still decays.
...
- AC4: ~~A manual authenticated invoke of `/api/cron/warmth` on production shall return a JSON body with numeric `processed`, `hot`, `warm`, `cold`, `unscored`.~~ **[AMENDED 2026-09-25:** returns numeric `processed`, `hot`, `warm`, `cold` — no `unscored` key**]**
...
- AC1: Warmth panel Hot bar fill shall be **`bg-status-hot`** (`--color-status-hot` #d0492f — founder override 2026-09-25; tokens are source of truth, Standing Decision 8 amended); Warm stays `bg-status-warm`; Cold stays `bg-status-cold`; ~~Unscored stays `bg-muted`~~ **[AMENDED 2026-09-25:** Unscored bar removed entirely — panel renders 3 bars**]**.
...
- AC5: `GET /api/dashboard/warmth` shall compute tier counts without loading every `warmth_score` row into JS when feasible — prefer head-count queries + total count (~~4× `count: "exact", head: true`~~ 3 tiers + total post-restructure); keep auth, owner check, required `waitlist_id`, and 30s cache headers. **[AMENDED 2026-09-25:** response shape `{ hot, warm, cold, total }` — no `unscored` count**]**
...
- **T5:** `dashboard/warmth/route.ts:38-47` full select → head counts; preserve JSON `{ hot, warm, cold, ~~unscored~~, total }` **[shape amended 2026-09-25: `{ hot, warm, cold, total }`]**.
...
- AC1: Unit tests for scoring/decay/tier shall cover: multi-signal totals; clamp 0/100; **sent/delivered do not reset decay**; day 59 no penalty; day 60 −25; 90+ → 0; ~~engaged score 0 → cold; never-engaged 0 → null~~ **[AMENDED 2026-09-25:** baseline 70 start, `assignTier` never `null`, decay clock = max(click, referral, signup)**]**; zero-click + old `created_at` still decays; referrals ×15; qual +8.
...
- AC5: Component tests for WarmthPanel shall cover: Hot bar class includes `bg-status-hot` (and not `bg-accent`); Warm/Cold ~~Unscored~~ **[Unscored class case removed 2026-09-25 — 3 bars]** classes; empty state em-dashes; free tier upgrade badge text; **[added 2026-09-25:** title `<Link>` href, `aria-hidden` action, meta `"{n} subscribers"` shown/hidden**]**.
...
- AC6: `.memory/MEMORY.md` shall record Epic 15 standing decisions (dropped signals, click-only decay, ~~force-cold-on-engaged-zero~~ **[superseded 2026-09-25 by restructure]**, vercel cron, badge-on-warmth-page).
